...
namespace EigenSinnTest {
  ...
  protected:
    void SetUp() override {
      ...
      input.resize(batch_size, cols);
      loss.resize(batch_size, cols);
      gamma.resize(cols);
      beta.resize(cols);

      loss.setValues({ {0.07951424, 0.39795890, 0.48816258, 0.58650136, 0.80818069},
        {0.33679566, 0.74452204, 0.24355969, 0.36228219, 0.69534987 }});

      input.setValues({ { 0.56279999, 0.93430001, 0.25929999, 0.79210001, 0.15889999},
    ...
    Tensor<float, 2, RowMajor> input, loss, output;
