...
namespace EigenSinnTest {
  ...
  class Pool2dRowMajor : public ::testing::Test {
  ...
  protected:
    ...
    DeviceTensor<float, 2, ThreadPoolDevice, RowMajor> output, dinput, fakeloss;
  ...
  TEST_F(Pool2dRowMajor, Validate) {
  ...
  TEST_F(Pool2dRowMajor, Forward) {
    ...
    Tensor<float, 2, RowMajor> expected = output.to_host();
    Tensor<float, 2, RowMajor> actual = DeviceTensor<float, 2, ThreadPoolDevice, RowMajor>(pl.get_output()).to_host();

    out_compare(expected, actual);
  }

  TEST_F(Pool2dRowMajor, Backward) {
