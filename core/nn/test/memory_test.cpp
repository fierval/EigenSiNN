...
namespace EigenSinnTest {
  ...
  TEST_F(Memory, FlatRowMajor) {
    Tensor<Tuple<Index, float>, 4, RowMajor> idx_tuples = cd.convInput->index_tuples();
  ...
  TEST_F(Memory, FlatColMajor) {
    Tensor<Tuple<Index, float>, 4> idx_tuples = cd.convInput->swap_layout().reshape(cd.convInput.dimensions()).index_tuples();

    DSizes<Index, 4> offsets{ 1, 2, 1, 3 };
    Index actual = to_flat_dim<Index, 4, ColMajor>(cd.convInput.dimensions(), offsets);
  ...
  TEST_F(Memory, OffsetColMajor) {
    ...
    DSizes<Index, 4> actual = from_flat_dim<Index, 4, ColMajor>(cd.convInput.dimensions(), idx);
