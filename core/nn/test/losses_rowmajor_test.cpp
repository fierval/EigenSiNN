...
namespace EigenSinnTest {
  ...
  class LossRowMajor : public ::testing::Test {
  protected:
    void SetUp() {
      ...
      fc = new Linear<float, ThreadPoolDevice, RowMajor>(cd.dims[1], cd.out_dims[1]);
    ...
    float loss;
    DeviceTensor<float, 2, ThreadPoolDevice, RowMajor> dloss;
    CommonData2d<ThreadPoolDevice, RowMajor> cd;
    Linear<float, ThreadPoolDevice, RowMajor>* fc;
    Input<float, 2, ThreadPoolDevice, RowMajor> inp;
  };

  TEST_F(LossRowMajor, MSE) {
    ...
    DeviceTensor<float, 2, ThreadPoolDevice, RowMajor> output(fc->get_output());

    MseLoss<float, float, 2, ThreadPoolDevice, RowMajor> loss_func;
  ...
  TEST_F(LossRowMajor, CrossEntropy) {
